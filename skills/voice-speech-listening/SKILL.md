...
For v0.9, declare `listening_provenance.source_of_voice` whenever voice is human, synthetic, hybrid, or unknown; include producer, provider/model identifiers when known, vocal-data disclosure status, synthetic disclosure, authority to speak, and unvoiced parties. A transcript is attributed text, never a `heard` claim about vocal sound. Route corpus questions to `corpus-listening`, and keep undisclosed vocal-data lineage unknown.
