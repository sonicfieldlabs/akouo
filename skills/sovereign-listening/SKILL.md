...
Current v0.9 producers should emit context v2 plus `route_decisions`, `listening_passes`, and `listening_provenance`. Mark covenant-closed apertures `withheld`, copy only category-level attributed absence into `honest_absences`, and keep `action_authority` distinct from the covenant: a covenant can narrow authority but cannot silently expand it. Put refusal, withholding, forgetting, and non-action in an addressable route decision with authority and receipt references. Withheld, unavailable, refused, not retained, forgotten, and epistemically undetermined remain different states.
