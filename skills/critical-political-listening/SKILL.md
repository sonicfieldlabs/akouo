...
For v0.9, audit the source of listening as well as the apparent source of sound. Ask which corpora, providers, filters, labor, permissions, jurisdictions, and institutional cuts condition the ear itself. Preserve the distinction between directly disclosed lineage, partial disclosure, unknown lineage, and non-applicability; a critical reading must not fabricate the evidence it criticizes.
...
- `corpus-listening` when critique depends on training, fine-tuning, retrieval, annotation, provider disclosure, licensing, labor, or jurisdiction
